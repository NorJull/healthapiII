...
public class AppointmentServiceImpl implements AppointmentService {
	...
	public Map<Integer,Integer> getNumberOfAvailableAppointments(int specialityId, int doctorId, int year, int month) {
		...
		if (doctorId == ALL_DOCTORS_ID) {
			...
			appointments = appointmentDAO.getRegisteredAppointmentsOfAllDoctors(specialityId,
					startDate, finishDate);
			
		} else {
			schedules = scheduleDAO.getScheduleByDoctorAndSpeciality(specialityId, doctorId);
			
			appointments = appointmentDAO.getRegisteredAppointmentsOfOneDoctor(specialityId, doctorId,
					startDate, finishDate);
		}
			...
			for (Schedule schedule : schedules) {
				...
				mapSchedule.put(schedule.getWeekDay(), value +
						schedule.getAppointmentsTurnOne() + schedule.getAppointmentsTurnTwo());
			}
					
			//Contendra el número de citas disponibles que el doctor tiene en cada día del mes
